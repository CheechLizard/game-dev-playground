...
| **Capture** | The game renders its own frame into a PNG through Lua, without accessibility or desktop APIs. The Lua tool verifies output and enforces a timeout. | `shared/framework/capture.lua`, `tools/screenshot.lua`, `--capture`, `F7` |
| **Capture series** | Numbered PNGs from the same simulation, advancing a fixed number of ticks between snapshots. | `--frames`, `--every` |
| **Stationary targets** | Bench enemies whose AI, motion, separation and knockback are paused while damage and hit effects still run. Enabled by default in the bench only. | `bench.dummyStill`, `run.stationaryEnemies` |
